...
function login(){
		...
		if(usuario != "" && password != "") {
			...
			logAjax.open('GET','http://192.168.0.31:88/PROYECTO2/php/login.php?u='+usuario+'&p='+password);
			logAjax.send();
...
function crearContactos(){
	...
	contactosAjax.open('GET','http://192.168.0.31:88/PROYECTO2/php/contactos.php');
	contactosAjax.send();
...
function cargarUsuario(){
	...
	usuarioAjax.open('GET','http://192.168.0.31:88/PROYECTO2/php/usuario.php?&id='+idUser);
	usuarioAjax.send();
...
function registrar(){
	...
	if(usuario != "" && password != "" && correo != "") {
			//REGISTRAR
			logAjax = new XMLHttpRequest();
			logAjax.open('GET','http://192.168.0.31:88/PROYECTO2/php/registro.php?u='+user+'&p='+pass+'&c='+correo);
			logAjax.send();
			logAjax.onreadystatechange = function(){
				if (logAjax.readyState==4 && logAjax.status == 200) {
					if (logAjax.responseText!="0") {
...
item = document.getElementById('foto');
			...
			//REGISTRAR
			logAjax = new XMLHttpRequest();
			logAjax.open('GET','http://192.168.0.31:88/PROYECTO2/php/validarFoto.php?id='+id);
			logAjax.send();
			logAjax.onreadystatechange = function(){
				if (logAjax.readyState==4 && logAjax.status == 200) {
...
function cargarFoto(){
	...
	fotoAjax.open('GET','http://192.168.0.31:88/PROYECTO2/php/foto.php?&id='+idUser);
	fotoAjax.send();
...
function cargarDatos(){
	...
	datosAjax.open('GET','http://192.168.0.31:88/PROYECTO2/php/datos.php?&id='+idUser);
	datosAjax.send();
...
function score(){
	...
	starAjax.open('GET','http://192.168.0.31:88/PROYECTO2/php/score.php?&id='+idUser+'&rate='+puntaje);
	starAjax.send();
